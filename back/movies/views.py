...
def delta_e_2000(lab1, lab2, kL=1, kC=1, kH=1):
    ...
    # C1, C2 계산
    C1 = cp.sqrt(a1**2 + b1**2)
    C2 = cp.sqrt(a2**2 + b2**2)
    
    # C평균 계산
    Cbar = (C1 + C2) / 2
    
    # G 계산
    G = 0.5 * (1 - cp.sqrt(Cbar**7 / (Cbar**7 + 25**7)))
    ...
    # C'1, C'2 계산
    C1_prime = cp.sqrt(a1_prime**2 + b1**2)
    C2_prime = cp.sqrt(a2_prime**2 + b2**2)
    
    # h'1, h'2 계산
    h1_prime = cp.degrees(cp.arctan2(b1, a1_prime)) % 360
    h2_prime = cp.degrees(cp.arctan2(b2, a2_prime)) % 360
    ...
    if C1_prime * C2_prime == 0:
        ...
        if cp.abs(h2_prime - h1_prime) <= 180:
    ...
    deltaH_prime = 2 * cp.sqrt(C1_prime * C2_prime) * cp.sin(cp.radians(deltah_prime) / 2)
    ...
    if C1_prime * C2_prime == 0:
        ...
        if cp.abs(h1_prime - h2_prime) <= 180:
    ...
    # 가중치 계산
    T = (1 - 0.17 * cp.cos(cp.radians(hbar_prime - 30)) +
         0.24 * cp.cos(cp.radians(2 * hbar_prime)) +
         0.32 * cp.cos(cp.radians(3 * hbar_prime + 6)) -
         0.20 * cp.cos(cp.radians(4 * hbar_prime - 63)))
    
    deltaTheta = 30 * cp.exp(-((hbar_prime - 275) / 25)**2)
    RC = 2 * cp.sqrt(Cbar_prime**7 / (Cbar_prime**7 + 25**7))
    SL = 1 + (0.015 * (Lbar_prime - 50)**2) / cp.sqrt(20 + (Lbar_prime - 50)**2)
    SC = 1 + 0.045 * Cbar_prime
    SH = 1 + 0.015 * Cbar_prime * T
    RT = -cp.sin(cp.radians(2 * deltaTheta)) * RC
...
def recommend_movies(request):
    try:
        ...
        for movie in matching_movies:
            ...
            if similarities:
                # Convert list of similarities to a CuPy array
                similarity_values = cp.array([s[0] for s in similarities])
                avg_similarity = cp.mean(similarity_values)
                
                dominance_values = cp.array([s[1] for s in similarities])
                weights = cp.array([1/s[0] if s[0] != 0 else 1 for s in similarities])
                weighted_dominance = cp.average(dominance_values, weights=weights)
                
                movie_scores.append((movie, float(cp.asnumpy(avg_similarity * weighted_dominance))))
